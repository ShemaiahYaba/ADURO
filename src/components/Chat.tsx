    ...
    <div className="flex h-dvh flex-col overflow-hidden bg-[var(--background)]">
      <header className="sticky top-0 z-20 flex shrink-0 items-center gap-3 border-b border-[var(--surface-elevated)] bg-[var(--surface)] px-4 py-3">
        ...
        <button
          ...
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-[var(--muted)] transition hover:bg-[var(--surface-elevated)] hover:text-[var(--foreground)] disabled:opacity-40"
        >
          <svg
            viewBox="0 0 24 24"
            className="h-5 w-5"
      ...
      {showNewChatConfirm && (
        ...
        >
          <div className="w-full max-w-sm rounded-2xl border border-[var(--surface-elevated)] bg-[var(--surface)] p-5 shadow-xl">
            <h2 id="new-chat-title" className="text-base font-semibold">
              Start a new conversation?
            </h2>
            <p className="mt-2 text-sm text-[var(--muted)]">
              Your current chat will be cleared. Your nickname stays the same.
            </p>
            <div className="mt-5 flex justify-end gap-2">
              <button
                type="button"
                onClick={() => setShowNewChatConfirm(false)}
                className="rounded-lg px-4 py-2 text-sm text-[var(--muted)] transition hover:bg-[var(--surface-elevated)] hover:text-[var(--foreground)]"
              >
                Cancel
              </button>
              <button
